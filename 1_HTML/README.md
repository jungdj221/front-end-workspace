...
- accept : 파일 선택할 때 보여지는 파일들의 종류를 지정할 수 있다.

6. **hidden** : 정보를 가지고 있어야 하지만 사용자에게 안 보이게 할 때 사용한다.
7. **submt** : 데이터를 전송할 때 사용하는 버튼이다.

- form 태그의 method 속성에 지정된 전송 방식으로 action 속성에 짖정한 서버로 입력한 값들을 전송한다.

8. **reset** : form 태그에서 입력한 값을 모두 초기화한다.
9. **button** : 기본적으로 가진 기능이 없음. 다른 기능과 연결해서 사용.
...
1. placeholder : 텍스트입력의 도움이 되는 힌트를 표시
2. autofocus : 해당 태그에 마우스 커스를 바로 표시
...
- 대표적인 블럭 태그는 div, p, pre, hr, h1~6, table 태그 등이 있다.
- 블럭 태그 안에는 블럭, 인라인 태그 모두 사용할 수 있다.

## 인라인 태그

- 내용에 해당하는 부분만 영역을 차지하는 요소
- 인라인태그의 왼쪽/오른쪽에 다른 요소가 올 수 있다.
